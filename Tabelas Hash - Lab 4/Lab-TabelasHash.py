...
for i in range(0, 4):
    ...
    for k in range(0, 50):
        ...
        if k == 49:                                     # Calcula a média das consultas
            for m in range(0, 50):                  
                soma = soma + vet_colisoes[m]
            media = soma / 50

        if k == 1:                                      # Calcula o valor máximo de consultas
            maior_colisao = aux
    ...
    with open(nome_arq, 'a') as arq:    
        arq.write(f'MEDIA {media}\n')                   # Escreve a média das consultas no arquivo
    with open(nome_arq, 'a') as arq:    
        arq.write(f'MAXIMO {maior_colisao}\n')          # Escreve o nnúmero máximo de consultas no arquivo

    media = 0                                           # Limpa o vetor e as variáveis para depois no proximo tamanho
    soma = 0
    maior_colisao = 0
    vet_colisoes.clear()
    vet.clear()                                         
